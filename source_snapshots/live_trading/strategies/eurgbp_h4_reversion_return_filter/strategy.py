...
class EURGBPH4ReversionReturnFilterStrategy:
    ...
    def update_h4_state(self, h4_df: pd.DataFrame):
        min_bars = max(config.RANGE_LOOKBACK + 5, config.ATR_PERIOD + 5)
        ...
        lookback_df = frame.iloc[-config.RANGE_LOOKBACK:]
        ...
        if range_size <= 0:
            self.mean_price = None
            return
        self.mean_price = self.range_low + range_size * 0.5
    ...
    def check_pre_alert(self, tick):
        ...
        if self._direction_allows("SELL") and allowed_side in ("SELL", "BOTH"):
            sell_distance = min(abs(self.range_high - tick.bid), abs(self.range_high - tick.ask))
            if (
                sell_distance <= distance
                and self.last_short_signal_bar != self.last_h4_bar
                and self.last_short_pre_alert_bar != self.last_h4_bar
            ):
                self.last_short_pre_alert_bar = self.last_h4_bar
                signal = self._build_sell_signal()
                return self._pre_alert_payload("SELL", sell_distance, signal)

        if self._direction_allows("BUY") and allowed_side in ("BUY", "BOTH"):
            buy_distance = min(abs(self.range_low - tick.bid), abs(self.range_low - tick.ask))
            if (
                buy_distance <= distance
                and self.last_long_signal_bar != self.last_h4_bar
                and self.last_long_pre_alert_bar != self.last_h4_bar
            ):
                self.last_long_pre_alert_bar = self.last_h4_bar
                signal = self._build_buy_signal()
                return self._pre_alert_payload("BUY", buy_distance, signal)
        return None
    ...
    def _build_buy_signal(self):
        ...
        tp_distance = (self.mean_price - expected_entry) * config.TAKE_PROFIT
        if tp_distance <= 0:
            return None
        return {
            "side": "BUY",
    ...
    def _build_sell_signal(self):
        ...
        tp_distance = (expected_entry - self.mean_price) * config.TAKE_PROFIT
        if tp_distance <= 0:
            return None
        return {
            "side": "SELL",
    ...
    def _pre_alert_payload(self, side, distance, signal):
        if signal is None:
            return None
        return {
            "strategy": config.STRATEGY_NAME,
            "side": side,
            "trigger": signal["expected_entry"],
    ...
    def _state_ready(self):
        return all(value is not None for value in (
            self.range_high, self.range_low, self.mean_price, self.atr,
        ))
